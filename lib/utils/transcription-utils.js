...
const normalizeHoundify = (evt, channel, language) => {
  ...
  if (evt.type) {
    ...
    /* SDK format: has Disambiguation/ResultsAreFinal fields */
    is_final = evt.ResultsAreFinal && evt.ResultsAreFinal[0] === true;
  ...
  const setChannelVarsForStt = (task, sttCredentials, language, rOpts = {}) => {
    ...
    if ('google' === vendor) {
      ...
      const {
        ...
        requestInfo, sampleRate, audioQueryAbsoluteTimeout,
      ...
      opts = {
        ...
        ...(audioQueryAbsoluteTimeout && {HOUNDIFY_AUDIO_QUERY_ABSOLUTE_TIMEOUT: audioQueryAbsoluteTimeout}),
        ...(eoqThreshold !== undefined && {HOUNDIFY_EOQ_THRESHOLD: eoqThreshold}),
        ...(vadStopThreshold !== undefined && {HOUNDIFY_VAD_STOP_THRESHOLD: vadStopThreshold}),
      };
    }
