...
class SingleDialer extends Emitter {
  ...
  async exec(srf, ms, opts) {
    ...
    opts.headers = {
      ...
      /* request encrypted media (SRTP) on the outbound leg for sip: URI targets;
         sbc-outbound honors X-Jambonz-SRTP when forwarding the call */
      ...('sip' === this.target.type && this.dialTask?.srtpEncryption &&
        {'X-Jambonz-SRTP': this.dialTask.srtpEncryption})
    };
    ...
    try {
      ...
      this.dlg = await srf.createUAC(uri, {...opts,  followRedirects: true, keepUriOnRedirect: true}, {
        cbRequest: (err, req) => {
          ...
          this.emit('callStatusChange', {
            ...
            sipStatus: 100,
            sipReason: 'Trying',
            sipEvidence: this._sipEvidence({
              messageKind: 'synthetic_status',
              method: 'INVITE',
              responseText: 'Trying',
        ...
        cbProvisional: (prov) => {
          const status = {
            ...
            sipReason: prov.reason,
            sipEvidence: this._sipEvidence({
              messageKind: 'response',
              method: 'INVITE',
              responseCode: prov.status,
              responseText: prov.reason
            })
          };
      ...
      const responseHeaders = this._extractSipEvidenceHeaders(this.dlg.res);
      this.emit('callStatusChange', {
        sipStatus: 200,
        sipReason: this.dlg.res.reason,
        callStatus: CallStatus.InProgress,
        sipEvidence: this._sipEvidence({
          messageKind: 'response',
          method: 'INVITE',
          responseCode: 200,
          responseText: this.dlg.res.reason
        }),
      ...
      if (this.killed) {
        ...
        this.emit('callStatusChange', {
          ...
          duration,
          sipEvidence: this._sipEvidence({
            messageKind: 'local_termination',
            method: 'INVITE',
            responseText: 'Request Terminated',
            evidenceQuality: 'inferred'
          })
        });
      ...
      this.dlg
        .on('destroy', (req) => {
          ...
          const sipEvidence = req?.method ? this._sipEvidence({
            messageKind: 'request',
            method: req.method
          }) : undefined;
      ...
      if (err instanceof SipError) {
        ...
        status.sipEvidence = this._sipEvidence({
          messageKind: 'response',
          method: 'INVITE',
          responseCode: err.status,
          responseText: err.reason
        });
        const responseHeaders = this._extractSipEvidenceHeaders(err.res);
        if (responseHeaders) status.headers = responseHeaders;
  ...
  _extractSipEvidenceHeaders(message) {
    const allowed = [
      'Reason',
      'X-Khomp-Analytics-CC',
      'X-Khomp-Analytics-CC-Pattern',
      'X-Khomp-Analytics-CC-Pattern-Locale',
      'X-Domu-Khomp-Carrier-Answered-At',
      'X-Domu-Khomp-Analytics-Detected-At'
    ];
    if (!message || typeof message.has !== 'function' || typeof message.get !== 'function') return null;
    /**
     * DOM-7573 temporary diagnostic. Khomp RADIUS accounting keys on Khomp's
     * own call id (e.g. `7A341CBC...`), which we do not receive today because
     * this allowlist drops every header except the six above. To learn whether
     * Khomp emits that call id in a header we could correlate on, log the full
     * set of header NAMES present on Khomp's response (names only -> no PII).
     * Gated by env and wrapped so it can never affect call handling. Note: this
     * observes headers as they reach the feature-server (i.e. AFTER the drachtio
     * SBC) -- if a header is present at the SBC but stripped before us, it will
     * not appear here, and the check must move to the SBC.
     */
    if (process.env.KHOMP_SIP_HEADER_DEBUG === '1' && message.headers) {
      try {
        this.logger.info({
          event: 'dom-7573-khomp-response-header-inventory',
          headerNames: Object.keys(message.headers),
          callSid: this.callSid,
          sbcCallid: this.callInfo && this.callInfo.sbcCallid
        }, 'DOM-7573: Khomp response header inventory (diagnostic)');
      } catch (err) {
        /* diagnostic must never disrupt the call */
      }
    }
    const headers = {};
    allowed.forEach((name) => {
      if (message.has(name)) headers[name] = message.get(name);
    });
  ...
  _sipEvidence({messageKind, method, responseCode, responseText, evidenceQuality = 'observed'}) {
    return {
      messageKind,
      method,
  ...
  _notifyCallStatusChange({callStatus, sipStatus, sipReason, duration, headers, sipEvidence}) {
    ...
    if (this.callInfo) {
      ...
      // CallInfo is reused across callbacks, so overwrite absent values rather
      // than leaking evidence from an earlier SIP message.
      this.callInfo.sipHeaders = headers;
      this.callInfo.sipEvidence = sipEvidence;
      try {
...
module.exports.SingleDialer = SingleDialer;
