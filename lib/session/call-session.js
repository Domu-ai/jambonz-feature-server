 ...
 */
  ...
  */
  ...
  async _lccDtmf(opts, callSid) {
     ...
     * are not being transcoded (rtpengine has no DSP to synthesize the tone into). We fall back
     * to SIP INFO only when we have no endpoint in the media path for the targeted leg, e.g. when
     * media has been released to the SBC.
    ...
    if (ep?.connected) {
      try {
        ...
        this.logger.error({err},
          'CallSession:_lccDtmf - error sending DTMF via send_dtmf, falling back to SIP INFO');
      }
    }

    /* fallback: signal the SBC to inject DTMF via SIP INFO (used when media is released
     * and we have no endpoint in the media path for the targeted leg)
     */
    try {
      const dlg = callSid === this.callSid ? this.dlg : this.currentTask.dlg;
      const res = await dlg.request({
